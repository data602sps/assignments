...
def exercise02(new_observations):
    ...
    knn.fit(X,y)
    iris_predictions = knn.predict(I)
...
def exercise04():
    ...
    # ------ Place code below here \/ \/ \/ ------
    socre = []
    X_train, X_test, y_train, y_test = tts(iris.data, iris.target, test_size = 0.2, random_state = 21, stratify = iris.target)
    for k in range(1,31):
        knn = KNN(n_neighbors =k)
        knn.fit(X_train, y_train)
        scores.append(knn.score(X_test, y_test))
        fig = go.Figure([go.Scater(x = range(1,31), y = scores)])
        fig.show()
        py.plot(fig, sharing = 'public')
        overfit_underfit_curve = py.plot(fig, sharing = 'public')
...
def exercise05():
    ...
    # ------ Place code below here \/ \/ \/ ------
    bostondf = pd.DataFrame(data = np.c_[boston['data']], columns = boston['feature_names'])
    bostont = pd.DataFrame(data = np.c_[boston['target']], colums = ['target'])
...
def exercise06():
    ...
    plt.show()
    x_ptration = np.array(bostondf['PTRATIO'])
